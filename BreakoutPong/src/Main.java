...
public class Main
{
	...
	private boolean stopper, over, starting;
	...
	public void init()
	{
		...
		r1 = new Paddle(WIDTH - 50, HEIGHT / 2 - 50, 25, 80, 0, 0, 0, "r1Tex");// arrow
																				// keys
		r2 = new Paddle(25, HEIGHT / 2 - 50, 25, 80, 0, 0, 0, "r2Tex");// w a s
																		// d
	...
	public void start()
	{
		...
		while (stopper == false)
		{
			...
			if (starting)
			{
				...
				ttf.drawString((WIDTH / 2) - 100, HEIGHT / 48, "Begin!", Color.magenta);
				try
				{
					Display.update();
					Thread.sleep(1500);
	...
	public void render()
	{
		...
		if (over == false)
		{
			ttf.drawString((WIDTH / 2) - 550f, HEIGHT / 16f, "Score: " + (maxScore - score1), Color.magenta);// score
																												// left
			ttf.drawString((WIDTH / 2) + 275f, HEIGHT / 16f, "Score: " + (maxScore - score2), Color.magenta);// score
	...
	public void gamePlay()
	{
		...
		// checks if all blocks are gone, makes ball and paddles go faster
		// afterwards
	...
	public void texture1()
	{
		...
		glTexCoord2f(1, 1);
		glVertex2f(0 + texture.getTextureWidth(), 0 + texture.getTextureHeight());// bottom
																					// right
																					// coord
		glTexCoord2f(0, 1);
	...
	public void ballTouchesPaddle(Ball ball)
	{
		...
		if (((Math.abs(ball.getX() - r2X) - 10) <= 5) && ((Math.abs(ball.getY() - r2Y) - 15)) <= 35) // paddle
																										// one
		{
			...
			ball.setRight(!right);
		} else if (((Math.abs(ball.getX() - r1X) - 10) <= 5) && ((Math.abs(ball.getY() - r1Y) - 15)) <= 35) // paddle
																											// two
		{
			Sound sound = new Sound();
			sound.playSound();
